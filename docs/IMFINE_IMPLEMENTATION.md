...
安装后的用户主工作流入口是 BMAD-style Agent / Workflow entries：
...
创建 run
  ...
  -> 当前会话 Orchestrator / Agent 按决策显式调用确定性 runtime 工具
...
在 imfine 中，这些能力已经沉淀为 BMAD-style Workflow / Skill package：
...
在 imfine 中，这些能力已经沉淀为 BMAD-style Agent package：
...
在 imfine 中，这些能力已经沉淀为 BMAD-style 观察型 Agent 和 Workflow：
...
- 安装后不再用单个旧 `~/.codex/skills/imfine/SKILL.md` 或 `~/.claude/commands/imfine.md` 承载主行为
...
- `provider-observations/` 保存 provider UI / 会话观察，作为诊断线索，不能替代 provider-origin receipt
...
当前主工作流公开入口是安装后的 BMAD-style Agent / Workflow entries：
...
其中 `resume` 只返回当前 run 的只读 orchestration 快照，不负责写状态、制造 blocked 或补 runtime evidence。`orchestrate` 只负责读取和校验 `orchestrator-session.json` 并物化 dispatch / registry / parallel artifacts，不自动推进执行。真正的写入型收敛动作由当前会话 Orchestrator / Agent 显式调用 `agent complete`、`commit`、`push`、`archive`、`reconcile`、`finalize` 等确定性工具完成。

`imfine-run` 调用的 runtime 工具动作只创建 run、物化上下文并返回当前 orchestrator 快照，不切换到另一套 planning runtime，也不自动推进执行。

普通用户主流程不需要知道 `orchestrate`、`agent complete`、`finalize` 等内部命令。它们只能由当前会话 Orchestrator protocol、runtime backend 或测试 harness 调用，并受 internal runtime guard 保护。
...
- `ux-designer`: UI、交互、用户旅程、可访问性和 UX acceptance 设计；仅在用户界面或体验风险相关 run 中由 Orchestrator 条件启用
...
当前实现也已经将 Superpowers / BMAD / imfine workflow 能力集中到 `src/core/skill-registry.ts`：
...
当前 registry 覆盖基础能力与增强能力，包括：

- 基础能力：`clarify`、`project-analysis`、`write-delivery-plan`、`execute-task-plan`、`tdd`、`systematic-debugging`、`parallel-agent-dispatch`、`code-review`、`archive-confirmation`
- 执行支撑：`implementation`、`verification`、`risk-review`、`merge`、`scope-control`、`documentation`、`project-knowledge`、`archive`、`harness-audit`
- 当前增强能力：`brainstorming`、`product-brief`、`validate-requirement`、`implementation-readiness`、`correct-course`、`retrospective`
...
- Superpowers-inspired：clarify、project-analysis、write-delivery-plan、execute-task-plan、tdd、systematic-debugging、parallel-agent-dispatch、code-review、archive-confirmation、brainstorming
- BMAD-inspired：Product Planner、Architect、Dev、QA、Reviewer、Risk Reviewer、Committer、Archive、Project Knowledge Updater、UX Designer、product-brief、validate-requirement、implementation-readiness、correct-course、retrospective
...
- `ux-designer` 已接入 runtime role contract；UI/体验类 run 可以由 Orchestrator 条件启用 `imfine-agent-ux-designer`
- skill 契约已经集中到 skill registry，`AgentRun.skills` 会校验 role 适配性并进入 evidence contract；`brainstorming`、`product-brief`、`validate-requirement`、`implementation-readiness`、`correct-course`、`retrospective` 已接入 runtime skill evidence contract
- `run` 创建后固定进入 `waiting_for_agent_output`
- `run` 不提供自动推进分支，也没有 plan-only 分支；它只创建 run、物化上下文并返回当前 run 的 orchestrator 快照
...
- `worktree prepare`、`commit run`、`push run`、`archive` 等确定性工具会记录 runtime action ledger，服务 true-harness evidence，而不是替 Orchestrator 决定下一步
...
- `plan` 不属于公开命令
- `run` 不包含自动推进主脑；后续推进由当前会话 Orchestrator / Agent 显式调用确定性 runtime 工具完成
- runtime 不再自己生成 task graph 主语义
- runtime 不从需求关键词推导产品形态或业务验收标准
- 新增 Workflow 必须进入 skill registry 和 evidence contract，不能只在安装层暴露
- 新增 Agent 必须进入 role registry、handoff validator 和 dispatch contract，不能只生成 `SKILL.md`
- `ux-designer` 是条件启用角色，只有 UI、交互、用户旅程或 UX acceptance 相关 run 才需要进入 dispatch
