...
`.claude-plugin/marketplace.json` 必须列出所有可安装 package：

- 至少 16 个基础 `src/imfine-skills/agents/imfine-agent-*`，当前增强名册为 17 个
- 至少 10 个基础 `src/imfine-skills/workflows/imfine-*`，当前增强名册为 16 个
- `imfine-init`
- `imfine-run`
- `imfine-status`
- `imfine-observe`
- `imfine-archive`
- `imfine-help` 如果仍作为公开 help skill 保留

marketplace 声明不得只包含快捷入口或 5 个核心 Agent。
...
2. 创建 `.claude-plugin/marketplace.json`，列出完整 package 清单。
...
- `.claude-plugin/marketplace.json` 列出完整 imfine Agent / Workflow / 快捷入口 package。
...
- 安装后存在 `~/.agents/skills/imfine-agent-*` skill directories，覆盖基础 16 个 Agent，并覆盖当前增强 Agent。
- 安装后存在 `~/.agents/skills/imfine-*` workflow / shortcut skill directories，覆盖基础 10 个 Workflow、当前增强 Workflow 和公共快捷入口。
- Claude 安装后存在对应 `~/.claude/commands/imfine-agent-*.md` 和 `~/.claude/commands/imfine-*.md` command pointers。
- Codex 不再依赖 `~/.codex/skills/imfine/SKILL.md` 作为主行为入口，而是通过 `~/.agents/skills/imfine-*` 发现 skills。
- `~/.imfine/runtime` 继续存在。
- 安装后的公开使用方式与 BMAD 类似：可直接调用 `imfine-agent-orchestrator`、`imfine-init`、`imfine-run`、`imfine-status`、`imfine-observe`、`imfine-archive`，也可直接调用完整 Agent / Workflow skills。
