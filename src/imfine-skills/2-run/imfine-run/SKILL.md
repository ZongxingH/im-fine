...
2. Decide whether discovery is required:
   - use `imfine-brainstorming` for open-ended product, UX, workflow, or unclear success criteria;
   - use `imfine-product-brief` when product scope or acceptance needs structure;
   - use `imfine-validate-requirement` before architecture or task graph when ambiguity remains;
...
6. Run `imfine-implementation-readiness` before dispatching Dev Agents.
...
10. If material assumptions change, run `imfine-correct-course` and re-run readiness before resuming.
