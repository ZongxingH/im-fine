...
assert.match(statusText, /Agent progress:/);
...
assert.ok(statusText.trim().split(/\r?\n/).length <= 60);
...
assert.doesNotMatch(statusText, /node .*imfine-runtime/);
const storyText = run(["status", "--story"], project);
assert.match(storyText, /Current wave:/);
assert.match(storyText, /Agent progress:/);
