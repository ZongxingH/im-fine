...
创建 run
  ...
  -> archive / reconcile / finalize 写出报告、final gates、acceptance matrix 和 true harness evidence
```
...
- `provider-receipts/` 记录 agent action 对应的 provider execution receipt，包括 provider agent/session、started/completed 时间、status 和 output path
...
- `acceptance-matrix.json` 同时表达产品形态、架构、持久化、前端、后端、测试、文档、git 和 archive evidence 的验收状态
...
- 将 provider receipt 从 `waiting_for_agent_output` 收敛为 `completed`、`blocked` 或 `failed`
...
当前 runtime 还会把 acceptance matrix 作为产品形态与工程交付的并行验收层，不用测试通过替代用户明确要求的产品形态。
...
- 生成 acceptance matrix、final report 和结构化 fix task
...
`acceptance-matrix.json` 是产品形态与工程交付的统一验收视图，覆盖：
...
- `required` 或 `negotiable`
- `demo-substitute` 或 `deviation`
- expected / observed
- QA/Review 是否接受
- evidence 文件列表

例如静态前端代替用户明确要求的小程序目录时，matrix 必须把它标为 `demo-substitute`；如果没有 QA/Review 接受，该 item 会阻断 final gate。

当前双 demo 对齐后的目标是同时吸收两个版本的优点：

- 早期 demo 的产品形态优点：`frontend/miniprogram`、多页 `frontend/admin`、SQLite/schema 级持久化证据
- 当前 demo1 的工程交付优点：可运行 REST 服务、真实测试、干净 git commit、轻依赖实现

如果实现选择“单静态页替代小程序目录”或“内存仓储替代 SQLite/MySQL 持久化”，必须在 acceptance matrix 和 final report 中明确标注为 `demo-substitute` 或 `deviation`。

当前 acceptance matrix 还会对 demo 应用交付做两类硬验收：
...
- provider capability 明确确认 `subagent_supported=supported`
...
- provider capability snapshot 与 provider execution receipt 已进入 true harness evidence，且 unknown/unsupported 不允许通过 true harness
...
- `agent complete` 会原子校验 handoff/evidence 并更新 provider receipt、agent-runs、parallel-execution、action-ledger
...
- `acceptance-matrix.json` 已用于区分产品形态不足、工程交付不足、文档不足、前端 contract 缺失、demo substitute 和 deviation
...
- test coverage 已覆盖 role registry 一致性、TaskGraph 负例、provider supported/unsupported/unknown、resolved_by_receipts、true harness 缺 receipt/wave/handoff/evidence/pre-archive 负例、status/gate matrix 状态输出、agent registry execution units、blocker matrix、project knowledge freshness、reconcile/finalize、agent complete、resume 幂等、acceptance matrix 对照 fixture 和真实双 demo replay
